...
public class ManageWarpInventory extends GUIChild {
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        if (e.getSlot() == 13) {
            GUIRegistry.register(new WarpNameEntryController(getPlayer(), ((ManageWarpInventoryController) controller).getPayload()));
            return;
        }
        if(e.getSlot() == 15) {
            GUIRegistry.register(new ManageGroupInventoryController(getPlayer(), ((ManageWarpInventoryController) controller).getPayload()));
            return;
        }
    }
