...
public class WarpMenuFragment implements ICommandFragment {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        boolean editAccess = PermissionHandler.getInstance().isAdmin(sender) || sender.hasPermission("sledgehammer.warp.edit");
        if(editAccess) {
            run(sender, true);
        } else {
    ...
    private void run(CommandSender sender, boolean permission) {
        SledgehammerNetworkManager.getInstance().send(new P2SWarpGUIPacket(sender.getName(),
                SledgehammerUtil.getServerFromSender(sender).getName(), permission));
    }
