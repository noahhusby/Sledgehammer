package com.noahhusby.sledgehammer.managers;
...
public class IncomingCommandHandler {
    ...
    public void incomingCommand(CommunicationMessage message) {
        ...
        switch (data[0]) {
            ...
            case "location":
                try {
                    TaskQueueManager.getInstance().queueTask(new TpllTask(data[3], Long.parseLong(data[2]),10000, TpllTask.CommandType.TPLL, data[4], data[5]));
                } catch (Exception e) {
    ...
    private boolean isGenuineRequest(String u) {
        try {
            Sledgehammer.logger.info(u+", "+ConfigHandler.authenticationCode);
